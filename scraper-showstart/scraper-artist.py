# 小河演出抓取：100条+详情页信息+字段清洗+JSON存储（小河.json）
import time
import json
import re
...
# 配置项（小河100条+2026全年+小河.json命名）
CONFIG = {
    "list_url": "https://www.showstart.com/event/list?cityCode=10&keyword=%E5%B0%8F%E6%B2%B3",
...
def main():
    print("🔍 启动秀动爬虫（小河100条+详情页+地址去查看地图版）")
    print("================================")
    ...
    try:
        ...
        print(f"📊 最终保存 {len(json_data)} 条2026年小河演出数据（含详情+风格标签）")
    ...
    print("================================")
    print("🔚 程序执行完毕（小河100条+详情页+地址去查看地图版）")
